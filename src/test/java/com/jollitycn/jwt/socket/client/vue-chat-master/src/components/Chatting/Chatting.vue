...
export default {
  ...
  mounted() {
    ...
    socket.on('user_join', (data) => {
      ...
      let ouser_join = document.createElement('div');
      ouser_join.className = 'user_join';
      ouser_join.innerText = data.userName + '上线了';
      this.oContent.appendChild(ouser_join);
      this.oContent.scrollTop = this.oContent.scrollHeight;
    });
    socket.on('user_left', (data) => {
      ...
      let ouser_join = document.createElement('div');
      ouser_join.className = 'user_left';
      ouser_join.innerText = data.userName + '下线了';
      this.oContent.appendChild(ouser_join);
      this.oContent.scrollTop = this.oContent.scrollHeight;
    });
...
if(this.$store && this.$store.state.name) {
  socket.emit('user_join', this.$store.state.name);
}else {
  console.log("服务器连接，请重新登录！");
}
</script>
